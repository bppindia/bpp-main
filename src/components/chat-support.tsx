...
export default function ChatSupport() {
    const [messages, setMessages] = useState<Message[]>([
        {
            role: "bot",
            content: "Hello! How can I help you today? Please select one of these common questions:",
        }
    ]);
    ...
    const handleQuestionClick = async (question: typeof chatData.questions[0]) => {
    ...
    const handleSuggestionClick = async (suggestion: string) => {
        ...
        setMessages(prev => [
            ...prev,
            { role: "bot", content: `Here's more information about ${suggestion}. [Additional content would be added here based on the suggestion]` }
        ]);
        setShowResolutionOptions(true);
        setCurrentSuggestions([]);
    };

    const handleResolutionClick = async (resolution: typeof chatData.resolutionOptions[0]) => {
        setMessages(prev => [...prev, { role: "user", content: resolution.text }]);
        await simulateTyping();
        setMessages(prev => [...prev, { role: "bot", content: resolution.response }]);
        setShowResolutionOptions(false);
    ...
    const handleCustomQuestion = async (input: string) => {
        ...
        if (matchingQuestion) {
            setMessages(prev => [
                ...
                {
                    role: "bot",
                    content: "I found this answer that might help:\n\n" + matchingQuestion.answer
                }
            ]);
            ...
            setMessages(prev => [
                ...
                {
                    role: "bot",
                    content: "I apologize, but I couldn't find a specific answer to your question. Let me connect you with our support team who can better assist you."
                }
            ]);
            setShowContactOptions(true);
        }
    ...
    return (
        <ExpandableChat size="md" position="bottom-right">
                    ...
                    {showContactOptions && !isTyping && (
                        <div className="flex flex-col gap-2 mt-4">
                            <Button
                                ...
                                onClick={() => window.open(`https://wa.me/${chatData.contactOptions.whatsapp}`)}
                            >
                            ...
                            <Button
                                variant="default"
                                className="flex items-center gap-2"
                                onClick={() => window.location.href = `mailto:${chatData.contactOptions.email}`}
                            >
                            ...
                            <Button
                                variant="default"
                                className="flex items-center gap-2"
                                onClick={() => window.location.href = `tel:${chatData.contactOptions.phone}`}
                            >
            ...
            <ExpandableChatFooter className="bg-muted/25">
                <form className="flex relative gap-2" onSubmit={async (e) => {
                    e.preventDefault();
