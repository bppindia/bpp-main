...
const mockDistrictData: { [state: string]: { [district: string]: DistrictData } } = {
    maharashtra: {
        jalgaon: {
            district: "Jalgaon",
            totalMembers: 12000,
...
const stateProjections: StateProjections = {
    ...
    dadra_and_nagar_haveli_and_daman_and_diu: { scale: 4000, center: [72.9, 20.3] },
...
const MapChart: React.FC<MapChartProps> = ({ state, dist }) => {
    ...
    const mapData = useMemo(() => {
        ...
        switch (normalizedState) {
            case 'andhra_pradesh': return andhra_pradesh;
            case 'arunachal_pradesh': return arunachal_pradesh;
            ...
            case 'uttar_pradesh': return uttar_pradesh;
            case 'uttarakhand': return uttarakhand;
            case 'west_bengal': return west_bengal;
            case 'andaman_and_nicobar_islands': return andaman_and_nicobar_islands;
            case 'chandigarh': return chandigarh;
            case 'dadra_and_nagar_haveli_and_daman_and_diu': return dadra_and_nagar_haveli_and_daman_and_diu;
            case 'delhi': return delhi;
            case 'jammu_and_kashmir': return jammu_and_kashmir;
    ...
    const getFillColor = (geo: GeographyProps['geography']): string => {
        ...
        if (selectedDistrict && properties.district?.toLowerCase() === selectedDistrict.toLowerCase()) {
    ...
    const handleDistrictClick = (geo: GeographyProps['geography']) => {
        ...
        const normalizedState = state.toLowerCase().replace(/\s+/g, '_');
        const mockData = mockDistrictData[normalizedState]?.[districtName.toLowerCase()];
                                            ...
                                            <TooltipTrigger asChild>
                                                <Geography
                                                    ...
                                                    style={{
                                                        ...
                                                        hover: {
                                                            fill:
                                                                dist && (geo.properties as GeoProperty).district?.toLowerCase() === dist.toLowerCase()
                                                                    ? '#0EA5E9'
                                                                    : '#60A5FA',
                                                            outline: 'none',
                                                            stroke: '#FFFFFF',
                        ...
                        <CardContent>
                            {loading ? (
                                ...
                                <div className="space-y-2">
                                    <p><strong>Total Members:</strong> {districtData.totalMembers.toLocaleString()}</p>
                                    <p><strong>Total Population:</strong> {districtData.totalPopulation.toLocaleString()}</p>
                                    <p><strong>Registered Voters:</strong> {districtData.registeredVoters.toLocaleString()}</p>
                                    <p><strong>Active Volunteers:</strong> {districtData.activeVolunteers.toLocaleString()}</p>
                                </div>
                            ) : (
