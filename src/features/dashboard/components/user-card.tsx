...
const userCardStyles: UserCardStyles = {
  [UserStatus.PROCESSING]: {
    ...
    badgeStyle: 'bg-red-100 text-red-700 border-red-200 dark:bg-red-900/50 dark:text-red-400 dark:border-red-800',
  },
  [UserStatus.APPROVED]: {
    [UserRole.MEMBER]: {
      ...
      badgeVariant: 'outline',
      badgeText: 'Verified',
      badgeStyle: 'bg-green-100 text-green-700 border-green-200 dark:bg-green-900/50 dark:text-green-400 dark:border-green-800',
    },
    [UserRole.PRIMARY_MEMBER]: {
      ...
      badgeStyle: 'bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/50 dark:text-blue-400 dark:border-blue-800',
    },
    [UserRole.ACTIVE_MEMBER]: {
      ...
      badgeStyle: 'bg-purple-100 text-purple-700 border-purple-200 dark:bg-purple-900/50 dark:text-purple-400 dark:border-purple-800',
    },
  },
} as const;
...
const UserCard = memo(({ dashboardData, isLoading }: UserCardProps) => {
  ...
  const renderUserInfo = () => {
    ...
    if (dashboardData.user.status === UserStatus.PROCESSING || !dashboardData.membership) {
      return (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
          <div>
          ...
          <div>
            ...
            <p className="truncate">
              {dashboardData.user.dateOfBirth ? new Date(dashboardData.user.dateOfBirth).toLocaleDateString() : 'N/A'}
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="truncate">{dashboardData.user.address.state || 'N/A'}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">District</p>
            <p className="truncate">{dashboardData.user.address.district || 'N/A'}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">City/Village</p>
            <p className="truncate">{dashboardData.user.address.cityOrVillage || 'N/A'}</p>
          </div>
        </div>
      )
    }
    ...
    return (
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
        <div>
          ...
          <p className="truncate font-medium">{dashboardData.membership?.number || 'N/A'}</p>
        </div>
        <div>
          <p className="text-sm text-muted-foreground">Membership Type</p>
          <p className="truncate font-medium">{dashboardData.user?.role || 'N/A'}</p>
        </div>
        <div>
          <p className="text-sm text-muted-foreground">Membership Validity</p>
          <p className="truncate font-medium">
            {dashboardData.membership?.expiryDate
              ? new Date(dashboardData.membership.expiryDate).toLocaleDateString()
              : 'N/A'}
          </p>
        </div>
        <div>
          <p className="text-sm text-muted-foreground">Wallet Balance</p>
          <p className="truncate font-medium">₹{dashboardData?.wallet?.balance || 0}</p>
        </div>
        <div>
          <p className="text-sm text-muted-foreground">Location</p>
          <p className="truncate font-medium">
            {dashboardData?.user?.address?.state || 'N/A'} ({dashboardData?.user?.address?.district || 'N/A'})
          </p>
        </div>
        <div>
          <p className="text-sm text-muted-foreground">Referral Code</p>
          <p className="truncate font-medium">{dashboardData?.referrals?.referralCode || 'N/A'}</p>
        </div>
        <div>
          <p className="text-sm text-muted-foreground">Total Referrals</p>
          <p className="truncate font-medium">{dashboardData?.referrals?.totalReferrals || 'N/A'}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      <Card className={cn('mb-6 overflow-hidden', cardStyle.bg, cardStyle.border)}>
          ...
          <div className="flex flex-col items-center gap-6 sm:flex-row sm:items-start">
            {isLoading ? (
              ...
              <Avatar className="h-24 w-24 shrink-0 sm:h-32 sm:w-32">
                <AvatarImage
                  src={typeof authUser?.profileImage === 'string' ? authUser.profileImage : undefined}
            ...
            <div className="flex-1 space-y-4 w-full">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                {isLoading ? (
                  ...
                  <h2 className="text-xl sm:text-2xl font-bold truncate">
                    {dashboardData.user.title} {dashboardData.user.firstName} {dashboardData.user.middleName ? `${dashboardData.user.middleName} ` : ''}{dashboardData.user.lastName}
                  </h2>
                )}
                {!isLoading && (
                  <Badge 
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
                {isLoading ? (
                  ...
                  <>
                    {(isPrimaryMember || isActiveMember) && dashboardData.membership?.number && (
                      <div className="flex flex-wrap items-center gap-2">
                        <Button
                          ...
                          className="w-auto"
                        >
                          <Eye className="h-4 w-4 sm:h-6 sm:w-6 mr-2" />
                          View Card
                        </Button>
                        <ReferralShare referralLink={dashboardData?.referrals?.referralLink || undefined} />
                        {(isPrimaryMember && hasEnoughReferrals) || isMembershipExpired ? (
                          <Button
                            variant="outline"
                            size="default"
                            onClick={() => {
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex items-center justify-center p-4">
            {dashboardData.membership?.cardUrl ? (
              ...
              <p className="text-muted-foreground">No membership card available</p>
            )}
          </div>
        </DialogContent>
      </Dialog>
